if(getRversion() >= "3.1.0") utils::globalVariables(c("myNorm","CellTypeMeans27K","CellTypeMeans.DPLFC"))

DLPFC.refbase <- function(beta=myNorm,
                          arraytype="450K")
{
    message("[===========================]")
    ...
    {
        load("CellTypeMeans.DLPFC.RData")    # 450K
        DMRs450K <- intersect(rownames(CellTypeMeans.DLPFC), rownames(beta))
        cellFrac <- projectWBC(beta[DMRs450K,],CellTypeMeans.DPLFC[DMRs450K,],lessThanOne=TRUE)
    }
